...
echo "Начата установка необходимого ПО"
echo "-----------------------------------------------------------------------------"
curl -s https://raw.githubusercontent.com/b4dcat404/crypto_deity/tools/install_ufw.sh | bash &>/dev/null
curl -s https://raw.githubusercontent.com/b4dcat404/crypto_deity/tools/install_docker.sh | bash &>/dev/null
# curl -s https://raw.githubusercontent.com/b4dcat404/crypto_deity/tools/install_node14.sh | bash &>/dev/null
...
if [ ! -e $HOME/metamask.txt ]; then
	echo "Файл с приватным ключем от от кошелька MetaMask отсутсвует"
  exit 1
fi
...
echo "Нода успешно запущена. Следуем дальше по гайду"
echo "-----------------------------------------------------------------------------"